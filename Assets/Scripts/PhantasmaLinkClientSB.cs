...
public class PhantasmaLinkClientSB : MonoBehaviour
{
    ...
    public bool RetryConnection { get; private set; } = false;
    ...
    {
        Debug.Log(String.Format("Checking if LinkClient Is Enabled:{0} ", this.Enabled));

        if (this.Enabled)
        {
            return;
        }
        Debug.Log("Enabling Phantasma Link Client...");
        this.Enabled = true;
    ...
    IEnumerator PingWallet()
    {
        ...
        while (PingConnection)
        {
            ...
            Task t = Task.Run(async () => await CheckConnection());
    ...
    {
        // Debug.Log(String.Format("Checking WebSocket connection...{0}", websocket.State));
        ...
        else if (websocket != null && websocket.State == WebSocketState.Closed)
        {
            ...
            var result = await ReconnectWithTimeout();
            if (result)
            {
                Debug.Log("Reconnected to the wallet successfully.");
                //Sometimes websocket state is still connecting, must be open state
                if (websocket.State == WebSocketState.Open)
                    this.Ready = true;
            }
            else
            {
                Debug.LogWarning("Failed to reconnect to the wallet.");
                this.PingConnection = false;
                await websocket.Close();
    ...
    //This should be redefined in a better way, but for now it works
    /// <summary>  
    /// Reconnects to the WebSocket with a timeout and retries.
    /// </summary>
    /// <param name="timeout">Timeout in milliseconds.</param>
    /// <param name="retries">Number of retries.</param>    
    /// /// <returns>True if reconnected successfully, false otherwise.</returns>
    private async Task<bool> ReconnectWithTimeout(int timeout = 5000, int retries = 10)
    {
        Debug.Log(String.Format("Attempting to reconnect... Retries left: {0}", retries));
        if (retries == 0 & websocket.State != WebSocketState.Open)
        {
            return false;
        }
        //Thinking of using events for this, but for now just using a simple timeout
        else if (this.RetryConnection)
        {
            ...
            //Wait for connect task to complete before recursion
            if (completedTask != timeoutTask)
                await connectTask;

            Task.Delay(timeout).Wait();
            Debug.LogWarning(String.Format("WebSocket connection timed out, retrying...{0}", retries));
            return await ReconnectWithTimeout(timeout, retries - 1);
        }
        return true;
    }
    public async void resetWebSocket()
    {
        if (websocket != null)
        {
            ...
            this.Enabled = false;
            await websocket.Close();
            Debug.Log(String.Format("WebSocket closed: {0}", websocket.State));
            websocket = null;
        }
    }
